...
$isForces = array('0'=>'默认更新','1'=>'强制更新');//更新模式
if ($type=='delete') {
	...
	$sql = "UPDATE `lord_game_room` SET `is_del` = 1 WHERE `id` = $id AND `modelId` = 91";
...
if ($type=='close') {
	...
	$room = $db->fetch(PDO::FETCH_ASSOC);
	if ( ! $room ) { echo json_encode(array('errno'=>9, 'error'=>"查询错误。")); exit; }
	$sql = "UPDATE `lord_game_room` SET `isOpen` = 0 WHERE `id` = $id AND `modelId` = 91";
	$res = $pdo->getDB(1)->exec($sql);
	$errno = 0; $error = "";
	if ( $res ) {
		//security
...
$data = array();
if ($type=='modify') {
	$sql = "SELECT * FROM `lord_game_room` WHERE `id` = $id AND `modelId` = 91";
...
if ($ispost) {
	$modelId = 91;
	$mode = isset($_REQUEST['mode'])?trim($_REQUEST['mode']):'广告位';
	...
	$isForce = isset($_REQUEST['isForce'])?intval(!!$_REQUEST['isForce']):0;
	...
	if ($type=='add') {
		$sql = "INSERT INTO `lord_game_room` (`isOpen`,`isMobi`,`verMin`,`modelId`,`mode`,`roomId`,`room`,`name`,`showRules`,`apkurl`,`isForce`,`appid`,`ver`,`vercode`,`bytes`,`desc`,`md5`,`package`,`sort`,`create_time`,`update_time`)
		VALUES ($isOpen,$isMobi,$verMin,$modelId,'$mode',$roomId,'$room','$name',$showRules,'$apkurl',$isForce,$appid,'$ver',$vercode,$bytes,'$desc','$md5','$package',$sort,$create_time,$update_time)";
		$res = $pdo->getDB(1)->exec($sql);
		$res = $id = $pdo->getDB(1)->lastInsertId();
	}
	if ($type=='modify') {
		$sql = "UPDATE `lord_game_room` SET `isOpen`=$isOpen,`isMobi`=$isMobi,`verMin`=$verMin,`modelId`=$modelId,`mode`='$mode',`roomId`=$roomId,`room`='$room',`name`='$name',`showRules`=$showRules,`apkurl`='$apkurl',`isForce`=$isForce,`appid`=$appid,`ver`='$ver',`vercode`=$vercode,`bytes`=$bytes,`desc`='$desc',`md5`='$md5',`package`='$package',`sort`=$sort,`update_time`=$update_time WHERE `id`=$id AND `modelId` = 91";
		$res = $pdo->getDB(1)->exec($sql);
	}
	$errno = 0; $error = "";
	if ( $res ) {
	...
	if ( !$errno ) {
		header('Location: lobbyARoomList.php');
	}
	else {
		echo $res;
		exit;
...
<fieldset>
	<legend>广告场 - <?php if($type=='add'){?>创建<?php }else{?>编辑<?php }?></legend>
	<form action="lobbyARoomAdd.php" method="post">
		...
		<table class="table table-bordered table-condensed table-hover">
			...
			<tr>
				<td style="width:70px;font-size:14px;">广告场apk-更新模式:</td><?php $var = 'isForce';?>
				<td><select name="<?=$var?>" class="span2"><?php foreach (${$var.'s'} as $k => $v) { echo "<option value='$k'".($data && $data[$var]==$k ? " selected='selected'" : "").">$v</option>"; } ?></select></td>
			</tr>
			<tr>
