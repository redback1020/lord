...
$rooms = array(1000,1001,1002,1003);
foreach ( $dds as $dd )
{
...
//chart1
cid = 'chart1'; ttl = '活跃明细 - 经典新手场'; lns = []; lbs = []; stl = 'filledCircle'; fmt = '%R';
<?php if (count($rd1000playing7)>1){ ?>lns.push(<?php echo (json_encode($rd1000playing7));?>);lbs.push('上周');<?php } ?>
<?php if (count($rd1000playing1)>1){ ?>lns.push(<?php echo (json_encode($rd1000playing1));?>);lbs.push('上日');<?php } ?>
<?php if (count($rd1000playing0)>1){ ?>lns.push(<?php echo (json_encode($rd1000playing0));?>);lbs.push('当日');<?php } ?>
makeChart(cid, ttl, lns, lbs, stl, fmt);
//chart2
cid = 'chart2'; ttl = '活跃明细 - 经典初级场'; lns = []; lbs = []; stl = 'filledCircle'; fmt = '%R';
<?php if (count($rd1001playing7)>1){ ?>lns.push(<?php echo (json_encode($rd1001playing7));?>);lbs.push('上周');<?php } ?>
<?php if (count($rd1001playing1)>1){ ?>lns.push(<?php echo (json_encode($rd1001playing1));?>);lbs.push('上日');<?php } ?>
<?php if (count($rd1001playing0)>1){ ?>lns.push(<?php echo (json_encode($rd1001playing0));?>);lbs.push('当日');<?php } ?>
makeChart(cid, ttl, lns, lbs, stl, fmt);
//chart3
cid = 'chart3'; ttl = '活跃明细 - 经典中级场'; lns = []; lbs = []; stl = 'filledCircle'; fmt = '%R';
<?php if (count($rd1002playing7)>1){ ?>lns.push(<?php echo (json_encode($rd1002playing7));?>);lbs.push('上周');<?php } ?>
<?php if (count($rd1002playing1)>1){ ?>lns.push(<?php echo (json_encode($rd1002playing1));?>);lbs.push('上日');<?php } ?>
<?php if (count($rd1002playing0)>1){ ?>lns.push(<?php echo (json_encode($rd1002playing0));?>);lbs.push('当日');<?php } ?>
makeChart(cid, ttl, lns, lbs, stl, fmt);
//chart4
cid = 'chart4'; ttl = '活跃明细 - 经典高级场'; lns = []; lbs = []; stl = 'filledCircle'; fmt = '%R';
<?php if (count($rd1003playing7)>1){ ?>lns.push(<?php echo (json_encode($rd1003playing7));?>);lbs.push('上周');<?php } ?>
<?php if (count($rd1003playing1)>1){ ?>lns.push(<?php echo (json_encode($rd1003playing1));?>);lbs.push('上日');<?php } ?>
<?php if (count($rd1003playing0)>1){ ?>lns.push(<?php echo (json_encode($rd1003playing0));?>);lbs.push('当日');<?php } ?>
makeChart(cid, ttl, lns, lbs, stl, fmt);
//chart5
cid = 'chart5'; ttl = '活跃明细 - 经典无限场'; lns = []; lbs = []; stl = 'filledCircle'; fmt = '%R';
<?php if (count($rd1006playing7)>1){ ?>lns.push(<?php echo (json_encode($rd1006playing7));?>);lbs.push('上周');<?php } ?>
<?php if (count($rd1006playing1)>1){ ?>lns.push(<?php echo (json_encode($rd1006playing1));?>);lbs.push('上日');<?php } ?>
<?php if (count($rd1006playing0)>1){ ?>lns.push(<?php echo (json_encode($rd1006playing0));?>);lbs.push('当日');<?php } ?>
makeChart(cid, ttl, lns, lbs, stl, fmt);
//jquery ready end
});
</script>
