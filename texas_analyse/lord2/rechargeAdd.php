...
if ($type=='delete') {
	$sql = "UPDATE `lord_list_recharge` SET `state` = 2 WHERE `id` = $id";
...
if ($type=='online') {
	$sql = "UPDATE `lord_list_recharge` SET `state` = 0 WHERE `id` = $id";
	$res = $pdo->getDB(1)->exec($sql);
	$errno = 0; $error = "";
	if ( $res ) {
		//security
...
if ($type=='offline') {
	$sql = "UPDATE `lord_list_recharge` SET `state` = 1 WHERE `id` = $id";
	$res = $pdo->getDB(1)->exec($sql);
	$errno = 0; $error = "";
	if ( $res ) {
		//security
...
$data = array();
if ($type=='modify') {
	$sql = "SELECT * FROM `lord_list_recharge` WHERE `id` = $id";
...
if ($ispost) {
	...
	if ($type=='add') {
		$sql = "INSERT INTO `lord_list_recharge` (`type`,`channel`,`title`,`fileId`,`value`,`price`,`store`,`is_onsale`,`onsale`,`sort`,`create_time`,`update_time`)
	...
	if ($type=='modify') {
		$sql = "UPDATE `lord_list_recharge` SET `type`='$itemtype',`channel`='$channel',`title`='$title',`fileId`=$fileId,`value`=$value,`price`=$price,`store`=$store,`is_onsale`=$is_onsale,`onsale`='$onsale',`sort`=$sort,`update_time`=$update_time WHERE `id`=$id";
		$res = $pdo->getDB(1)->exec($sql);
	}
	$errno = 0; $error = "";
	if ( $res ) {
	...
	if ( !$errno ) {
		header('Location: rechargeList.php');
	}
	else {
		echo $res;
		exit;
...
<fieldset>
	<legend>充值项 - <?php if($type=='add'){?>创建<?php }else{?>编辑<?php }?></legend>
	<form action="rechargeAdd.php" method="post">
		...
		<table class="table table-bordered table-condensed table-hover">
			...
			<tr>
				<td style="width:70px;font-size:14px;">货币数:</td>
