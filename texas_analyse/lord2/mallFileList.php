...
if ($isAjax) {
	...
	$where = "1=1 ";
	if ($path == 'all') $where .= " AND f.`path` IN ('".join("','", array_keys($pathes))."')";
	if ($path != 'all') $where .= " AND f.`path`='".$path."'";
	if ($version != 'all') $where .= " AND f.`version`={$version}";
	$sql = "SELECT f.* FROM `lord_game_file` f WHERE {$where} ORDER BY f.`id` DESC LIMIT {$pageIndex}, {$pageSize}";
		...
		if(result!=null && result!=""){
			...
			for(var i=0;i<dataList.data.length;i++){
				...
				if(o.ver_del>0){
					...
					dataListHtml += "<td><a href='#' onclick='ajaxto(this,\"mallFileAdd.php?type=delete&id="+o.id+"\")'>删除</a>&nbsp;&nbsp;<a href='#' onclick='linkto(this,\"mallFileAdd.php?type=modify&id="+o.id+"\")'>修改</a><br><a href='#' onclick='viewimg(this,\""+o.path+"/"+o.fileid+"\")'>查看图片</a></td>";
				}
				dataListHtml += "</tr>";
			}
...
<fieldset>
	...
	<div class="row">
		...
		<div span="span1" style="float:right;">
			...
			<input type="button" value="创&nbsp;&nbsp;建" onclick="linkto(this,'mallFileAdd.php')" class="btn"/>
		</div>
	</div>
</fieldset>
