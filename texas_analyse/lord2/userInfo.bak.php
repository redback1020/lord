	...
	$.ajax({
		...
		success:function(data){ 
			...
			for(var i=0;i<dataList.data.length;i++){
				...
				dataListHtml += "<td>"+(o.uuid.length>30?'':o.uuid)+"</td>";
				...
				dataListHtml += "<td>"+o.last_ip+"</td>";  
				dataListHtml += "<td>"+(o.is_tv==1?"TV":"MP")+"</td>";
				dataListHtml += "<td>"+o.channel+"</td>";
				dataListHtml += "<td>正常</td>"; 
				var time = "<?=time()?>";
				var sign = "<?=md5('qwe!@#321'.time())?>";
				dataListHtml += "<td style='display:none'><a href='userinfoWechat.php?time="+time+"&sign="+sign+"&type=uid&uid="+o.uid+"'>其他信息</a>&nbsp;&nbsp;</td>"; 
				dataListHtml += "</tr>";
			}
...
<body>
	...
	<div class="span2">
		...
		<?php
			...
			$pdo = new DB();
			$db = $pdo->getDB();
			$sql = "SELECT `channel` FROM `lord_game_user` WHERE channel !='' and is_tv=1 GROUP BY `channel`";
			$row = $db -> query($sql)-> fetchAll();
			foreach($row as $val){
				$str1.= '<option value="'.$val['channel'].'" name="tv">'.$val['channel'].'</option>';
			}
			$sql = "SELECT `channel` FROM `lord_game_user` WHERE channel !='' and is_tv=0 GROUP BY `channel`"; 
			$row = $db -> query($sql)-> fetchAll();  
			foreach($row as $val){
	...
	<div class="span2">
		<label>充值总额</label>
		<input style="height:30px;" class="span2" type="text" id="min"/>
		<input style="height:30px;" class="span2" type="text" id="max"/>
	</div>
	<div class="span2">
		<label>注册时间：</label>
	...
	<div span="span1" style="float:right;">
		...
		<table class="table table-bordered table-condensed table-hover">
			<tr class="info">
				...
				<td nowrap id="matcha"><strong><a style="cursor: pointer" onClick="query('a.match','matcha')">play</a></strong></td>
				<td nowrap id="matchd" style="display:none"><strong><a style="cursor: pointer" onClick="query('a.match','matchd')">play</a></strong></td>
				...
				<td nowrap><strong>上次登录IP</strong></td> 
				<td nowrap><strong>上次站点</strong></td> 
				<td nowrap><strong>上次渠道</strong></td> 
				<td nowrap><strong>状态</strong></td> 
				<td nowrap style="display:none"><strong>操作</strong></td> 
			</tr> 
			<tbody id="dataList">
			</tbody>
		</table>
