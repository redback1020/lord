...
if ($isAjax) {
	...
	$sql = "SELECT * FROM `lord_user_inbox` WHERE {$where} {$order} LIMIT {$pageIndex}, {$pageSize}";
	// echo $sql;
	$res = $db->query($sql)->fetchAll();
	$res = ( $res && is_array($res) ) ? $res : array();
	foreach ( $res as $k => $v )
	...
	$array['data'] = $res;
	$sql = "SELECT count(*) as cn FROM `lord_user_inbox` WHERE {$where} ";
	...
	$.post("userInboxList.php",{
		...
		if(result!=null && result!=""){
			...
			for(var i=0;i<dataList.data.length;i++){
				...
				dataListHtml += "<td><a href='#' onclick='ajaxto(this,\"userMailAdd.php?type=delete&id="+o.id+"\")'>删除</a>&nbsp;&nbsp;<a href='#' onclick='linkto(this,\"userMailAdd.php?type=modify&id="+o.id+"\")'>修改</a><br><a href='#' onclick='viewimg(this,"+o.fromuid+")'>查看图片</a></td>";
				dataListHtml += "</tr>";
			}
	...
	<div class="row">
		...
		<div class="span2">
			...
			<select class="span2" id="state">
				...
				<option value="2">已读-未处理</option>
				<option value="3">已读-已处理</option>
			</select>
		</div>
