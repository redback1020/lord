		...
		<div class="row">
			...
			<div class="span2">
				...
				//	if($adm_username!='all')$where = " and channel = '".$adm_username."'";
					if($data_priv!='all'){substr_count($data_priv,",")>0?$where = " and channel in (".$data_priv.")":$where = " and channel = '".$data_priv."'";}
					$sql = "SELECT `channel` FROM `lord_game_user` WHERE channel !='' and is_tv=1".$where." GROUP BY `channel`";
					...
					$sql = "SELECT `channel` FROM `lord_game_user` WHERE channel !='' and is_tv=0".$where." GROUP BY `channel`";
					$row = $db -> query($sql)-> fetchAll();
					foreach($row as $val){
						$str2.= '<option value="'.$val['channel'].'" name="mp">'.$val['channel'].'</option>';
					}

					$sql = "SELECT `channel` FROM `lord_game_user` WHERE channel !=''".$where." GROUP BY `channel`";
					$row = $db -> query($sql)-> fetchAll();
					foreach($row as $val){
	...
	<div>
		<table class="table table-bordered table-condensed table-hover">
			<tr class="info">
				...
				<td nowrap id="matchesa"><strong><a style="cursor: pointer" onClick="query('a.matches','matchesa')">matches</a></strong></td>
				<td nowrap id="matchesd" style="display:none"><strong><a style="cursor: pointer" onClick="query('a.matches','matchesd')">matches</a></strong></td>
