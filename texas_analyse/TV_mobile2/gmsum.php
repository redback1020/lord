 ...
 <body>
	...
	<div >
		...
		<div class="row">
			...
			<?php
			//$sql = "select * from mobile_user  where channel != '' group by channel";
		        $sql = "select distinct channel from mobile_user where channel != ''";
	...
	$finish = $end ;
	//注册用户数
	$sql = "select count(distinct m.uid) as cn,date(a.add_time) as dd from mobile_analyse a, mobile_user m where a.uid = m.uid and m.is_robot = 0 and a.add_time>='".$start."' and a.add_time<'".$end."'".$where." group by dd";
	//echo $sql;
	$row = $db -> query($sql) -> fetchAll(); 
...
//echo "</pre>";
	...
	while($begin<$finish){
		...
		$sql = "select count(distinct m.uid) as cn, add_time from mobile_game_login m where  add_time>='".$start2."' and add_time<'".$start3."' and reg_time>='".$start1."' and reg_time<'".$start2."'".$where."";
		//echo $sql.'<br>';
		$row = $db -> query($sql)->fetchAll();
		foreach($row as $val){
		...
		//当日转换
		$sql = "select count(distinct m.uid) as cn, add_time from mobile_game_login m where  add_time>='".$start1."' and add_time<'".$start2."' and reg_time>='".$start1."' and reg_time<'".$start2."'".$where."";
		//echo $sql.'<br>';
		$row = $db -> query($sql)->fetchAll();
		foreach($row as $val){
		...
		//当日注册付费用户
		$sql = "select count(distinct(m.uid)) as cn,date(a.add_time) as dd from ((SELECT uid,add_time FROM mobile_charge where sts = 1)UNION ALL (SELECT uid,add_time FROM mobile_charge_log where money>0 and (misc='taobao' or misc='web')))c, mobile_user m , user_analyse a where c.uid = m.uid and m.uid = a.uid and a.add_time>='".$start1."' and a.add_time<'".$start2."'".$where." and c.add_time>='".$start1."' and c.add_time<'".$start2."'";
		// echo $sql;
		$row = $db -> query($sql)->fetchAll();
		foreach($row as $val){
	...
	//活跃用户数
	$sql = "select count(distinct(m.uid)) as cn, date(add_time) as dd from mobile_game_login m where  m.add_time>='".$start."' and m.add_time<'".$end."'".$where." group by dd";
	$row = $db -> query($sql)->fetchAll();
	foreach($row as $val){
	...
	//当日所有付费用户
	$sql = "select count(distinct(m.uid))as cn,date(c.add_time) as dd from ((SELECT uid,add_time FROM mobile_charge where sts = 1))c, mobile_user m  where c.uid = m.uid and c.add_time>='".$start."' and c.add_time<'".$end."'".$where." group by dd";
	...
	//当日付费总额
	$sql = "select sum(c.in_money) as cn,date(c.add_time) as dd from ((SELECT uid,add_time,in_money FROM mobile_charge where sts = 1))c, mobile_user m  where c.uid = m.uid and c.add_time>='".$start."' and c.add_time<'".$end."'".$where." group by dd";
	$row = $db -> query($sql)->fetchAll();
	foreach($row as $val){
