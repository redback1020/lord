		...
		success:function(data){ 
			var dataList=eval("("+data+")");
				...
				for(var i=0;i<dataList.data.length;i++){
					...
					dataListHtml += "<td><a href='userinfoWechat.php?time="+time+"&sign="+sign+"&type=uid&uid="+o.uid+"'>其他信息</a>&nbsp;&nbsp;<a href='setVip.php?time="+time+"&sign="+sign+"&uid="+o.uid+"'>VIP</a></td>"; 
		...
		<div class="row">
			...
			<div class="span2">
				...
				<?php
					...
					$str1 = $str2 = $str = ""; 
					$sql = "SELECT `channel` FROM `mobile_user` WHERE channel !='' and is_tv=1 GROUP BY `channel`";
					...
					$sql = "SELECT `channel` FROM `mobile_user` WHERE channel !='' and is_tv=0 GROUP BY `channel`"; 
					$row = $db -> query($sql)-> fetchAll();  
					foreach($row as $val){
