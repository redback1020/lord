...
$max = $_REQUEST['max']==""?9999999:$_REQUEST['max'];
  ...
  if($channel != 'all') {
   ...
   if($data_priv!='all'){substr_count($data_priv,",")>0?$where .= " and channels in (".$data_priv.")":$where .= " and channels = '".$data_priv."'";}
	
  // if($data_priv!='all'){$where .= " and channels in (".$data_priv.")";}
  if($start != '')
