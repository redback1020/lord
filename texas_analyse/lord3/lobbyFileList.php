...
if ($isAjax) {
	...
	$where = "1=1 ";
	if ($path == 'all') $where .= " AND (f.`path`='lobby_navi' OR f.`path`='lobby_room' OR f.`path`='lobby_roompop' OR f.`path`='lobby_roomload')";
	if ($path != 'all') $where .= " AND f.`path`='".$path."'";
	if ($version != 'all') $where .= " AND f.`version`={$version}";
	$sql = "SELECT f.*, t.`name`, t.`sort` FROM `lord_game_file` f LEFT JOIN `lord_game_room` t ON f.`fileid` = t.`roomId` WHERE {$where} ORDER BY t.`sort` ASC, f.`fileid` DESC LIMIT {$pageIndex}, {$pageSize}";
	$res = $db->query($sql)->fetchAll();
		...
		if(result!=null && result!=""){
			...
			for(var i=0;i<dataList.data.length;i++){
				...
				if(o.ver_del>0){
					...
					dataListHtml += "<td><a href='#' onclick='ajaxto(this,\"lobbyFileAdd.php?type=delete&id="+o.id+"\")'>删除</a>&nbsp;&nbsp;<a href='#' onclick='linkto(this,\"lobbyFileAdd.php?type=modify&id="+o.id+"\")'>修改</a><br><a href='#' onclick='viewimg(this,\""+o.path+"/"+o.fileid+"\")'>查看图片</a></td>";
				}
				dataListHtml += "</tr>";
			}
...
<fieldset>
	...
	<div class="row">
		...
		<div span="span1" style="float:right;">
			...
			<input type="button" value="创&nbsp;&nbsp;建" onclick="linkto(this,'lobbyFileAdd.php')" class="btn"/>
		</div>
	</div>
</fieldset>
