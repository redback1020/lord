...
if ($isAjax) {
	...
	$sql = "SELECT * FROM `lord_list_trialcd` WHERE {$where} ORDER BY `sort`, `id` DESC LIMIT {$pageIndex}, {$pageSize}";
	// echo $sql;
	...
	$array['data'] = $res;
	$sql = "SELECT count(*) as cn FROM `lord_list_trialcd` WHERE {$where} ";
	...
	$.post("trialcdList.php",{
		...
		if(result!=null && result!=""){
			...
			for ( var i=0; i<dataList.data.length; i++ ) {
				...
				dataListHtml += "<td>"+o.count+"</td>";
				dataListHtml += "<td>"+o.cooldown+"</td>";
				dataListHtml += "<td>"+o.sort+"</td>";
				dataListHtml += "<td>"+o.create_time+"</td>";
				dataListHtml += "<td>"+o.update_time+"</td>";
				dataListHtml += "<td><a href='#' onclick='ajaxto(this,\"trialcdAdd.php?type=delete&id="+o.id+"\")'>删除</a>&nbsp;&nbsp;<a href='#' onclick='linkto(this,\"trialcdAdd.php?type=modify&id="+o.id+"\")'>修改</a></td>";
				dataListHtml += "</tr>";
			}
...
<fieldset>
	...
	<div class="row">
		...
		<div span="span1" style="float:right;">
			...
			<input type="button" value="创&nbsp;&nbsp;建" onclick="linkto(this,'trialcdAdd.php')" class="btn"/>
		</div>
	</div>
</fieldset>
