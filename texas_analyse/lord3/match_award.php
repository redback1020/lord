...
$searchs = array(
	// 'dateid'	=>array('name'=>'日期','int'=>1,'typ'=>'date','rel'=>0,'all'=>0),
...
// $dateid = isset($Q['dateid']) && $Q['dateid'] ? intval(str_replace('-','',$Q['dateid'])) : intval(date("Ymd"));
$apitable= 'lord_user_award';
$roomIds = array();
$sql = "SELECT `roomId`,`room` FROM `lord_game_room` WHERE `modelId` = 3 ORDER BY `sort`";
$ret = $db->query($sql)->fetchAll(PDO::FETCH_ASSOC);
if ( !$ret ) $ret = array();
foreach ( $ret as $k => $v ) {
	$roomIds[$v['roomId']] = $v['room'];
}
...
if ( $apitype == 'search' ) {
	...
	foreach ( $res as $k => $v )
	{
		$res[$k]['create'] = $v['create'] ? date("m-d H:i:s", $v['create']) : "&nbsp;";
		$res[$k]['update'] = $v['update'] ? date("m-d H:i:s", $v['update']) : "&nbsp;";
	}
	$json['data_res'] = $res;
...
function findit( is_next ) {
	...
	$.post("<?=$apiname?>.php?apitype=search&per_page="+per_page+"&cur_page="+cur_page, {
		...
		for ( var i=0; i<datalist.data_res.length; i++ ) {
			...
			datahtml += "<td>"+o.mobi+"</td>";
			datahtml += "<td>"+o.roomId+"</td>";
			datahtml += "<td>"+o.room+"</td>";
			datahtml += "<td>"+o.gameId+"</td>";
			datahtml += "<td>"+o.award+"</td>";
			datahtml += "<td>"+o.state+"</td>";
			datahtml += "<td>"+o.create+"</td>";
			datahtml += "<td>"+o.update+"</td>";
			datahtml += "</tr>";
		}
