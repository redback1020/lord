...
if($type == "day"){
	...
	$sql = "select max(num) as snum,count(*) as cn,date(add_time) as dd from lord_online where add_time >= '".$start."' and add_time<'".$date." 23:59:59' group by dd order by dd desc"; 
	$row = $db -> query($sql) -> fetchAll();
	foreach($row as $val){
	...
	$sql = "select max(`allOnline`) as snum,count(*) as cn,dateid as dd from lord_online_detail where dateid >= $startid and dateid< $endid group by dd order by dd desc";  
	$row = $db -> query($sql) -> fetchAll();
	foreach($row as $val){
		$dd = str_split($val['dd'], 2);
