...
if ($isAjax) {
	...
	$sql = "SELECT * FROM `lord_game_notice` WHERE {$where} ORDER BY `channel` ASC, `sort` ASC, `id` DESC LIMIT {$pageIndex}, {$pageSize}";
	$res = $db->query($sql)->fetchAll();
	$res = ( $res && is_array($res) ) ? $res : array();
	foreach ( $res as $k => $v )
	{
		$res[$k]['start_date'] = $v['start_time'] ? date("Y-m-d H:i:s", $v['start_time']) : "";
		$res[$k]['end_date'] = $v['end_time'] ? date("Y-m-d H:i:s", $v['end_time']) : "";
	...
	$array['data'] = $res;
	$sql = "SELECT count(*) as cn FROM `lord_game_notice` WHERE {$where} ";
...
function viewimg( o, id ) {
	...
	var _html = '<tr class="table-body viewimg"><td colspan="10">';
		_html+= '<a href="http://gt2.youjoy.tv/ddzgamefile/topic_notice/'+id+'.png" target="_blank"><img src="http://gt2.youjoy.tv/ddzgamefile/topic_notice/'+id+'.png" style="height:100px;" /></a>';
		_html+= '　';
		_html+= '<a href="http://gt2.youjoy.tv/ddzgamefile/topic_notice/'+id+'.png" target="_blank"><img src="http://gt2.youjoy.tv/ddzgamefile/topic_notice/'+id+'.png" style="height:100px;" /></a>';
		_html+= '</td></tr>';
	...
	$.post("noticeList.php",{
		...
		if(result!=null && result!=""){
			...
			for(var i=0;i<dataList.data.length;i++){
				...
				dataListHtml += "<td>"+o.start_date+"<br>"+o.end_date+"</td>";
				...
				dataListHtml += "<td><a href='#' onclick='ajaxto(this,\"noticeAdd.php?type=delete&id="+o.id+"\")'>删除</a>&nbsp;&nbsp;<a href='#' onclick='ajaxto(this,\"noticeAdd.php?type=offline&id="+o.id+"\")'>下架</a>&nbsp;&nbsp;<a href='#' onclick='linkto(this,\"noticeAdd.php?type=modify&id="+o.id+"\")'>修改</a></td>";
				dataListHtml += "</tr>";
			}
...
<fieldset>
	...
	<div class="row">
		...
		<div span="span1" style="float:right;">
			...
			<input type="button" value="创&nbsp;&nbsp;建" onclick="linkto(this,'noticeAdd.php')" class="btn"/>
		</div>
	</div>
</fieldset>
