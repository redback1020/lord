...
if ($ispost) {
	...
	if ( !$errno ) {
		header('Location: insideFileList.php');
	}
	else {
		echo $res;
		exit;
...
<fieldset>
	<legend>特殊用图 - <?php if($type=='add'){?>创建<?php }else{?>编辑<?php }?></legend>
	<form action="insideFileAdd.php" method="post">
		...
		<table class="table table-bordered table-condensed table-hover">
			...
			<tr>
				<td style="width:70px;font-size:14px;">位置编号:</td>
				<td><input name="fileid" value="<?php if($data){echo $data['fileid'];} ?>" type="text" class="in_t" />目前版本不要写除了0之外的数字</td>
			</tr>
			<tr>
				<td style="width:70px;font-size:14px;">图片上传:</td>
				<td>暂不支持，目前需要手动上传到CDN的目录<br/>
					inside_right 内页右侧图<br/>
					inside_banner (暂不支持)<br/>
					prize_real 实物奖品图<br/>
				</td>
			</tr>
			<tr>
