...
$searchs = array(
	...
	'hourid'	=>array('name'=>'小时','int'=>1,'typ'=>'select','rel'=>0,'all'=>1),
	'typeid'	=>array('name'=>'类型','int'=>1,'typ'=>'select','rel'=>0,'all'=>1),
...
$typeid = isset($Q['typeid']) && isset($typeids[$Q['typeid']]) ? intval($Q['typeid']) : 'all';
...
function findit( is_next ) {
	...
	$.post("<?=$apiname?>.php?apitype=search&per_page="+per_page+"&cur_page="+cur_page, {
		...
		for ( var i=0; i<datalist.data_res.length; i++ ) {
			...
			datahtml += "<td>"+(((o.typeid > 50 && o.typeid < 91 && ((o.typeid == 54 && o.moneyid == '1') || o.typeid != 54) )||o.typeid == 92)?'-':'')+o.moneynum+"</td>";
