...
if($type == "day"){
	...
	$sql = "select sum(`allOnline`) as snum,count(*) as cn,dateid as dd from lord_online_detail where dateid >= $startid and dateid< $endid group by dd order by dd desc"; 
	$row = $db -> query($sql) -> fetchAll();
	foreach($row as $val){
	...
	$row = $db -> query($sql) -> fetchAll();
	foreach($row as $val){
		$a_[] = array(date("H:i",$val['ut']),intval($val['allOnline']));
	}
	// $a_[] = array(date("H").':55','-');
	...
	$row = $db -> query($sql) -> fetchAll();
	foreach($row as $val){
		$a2_[] = array(date("H:i",$val['ut']),intval($val['allOnline']));
	}
	// $a2[] = array(date("H").':55','-');
	...
	$row = $db -> query($sql) -> fetchAll();
	foreach($row as $val){
		$a3_[] = array(date("H:i",$val['ut']),intval($val['allOnline']));
	}
	// $a3_[] = array(date("H").':55','-');
}
...
$(document).ready(function(){
	...
	lns = []; lbs = [];
	<?php if (count($a3_)>1){ ?>lns.push(<?php echo (json_encode($a3_));?>);lbs.push('上周');<?php } ?>
	<?php if (count($a2_)>1){ ?>lns.push(<?php echo (json_encode($a2_));?>);lbs.push('昨日');<?php } ?>
	lns.push(<?php echo (json_encode($a_));?>);lbs.push('当日');
