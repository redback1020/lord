...
$itemtypes = array('money2coins'=>'人民币充乐豆');
$ut_now = time();
...
if ($isAjax) {
	...
	$sql = "SELECT * FROM `lord_list_recharge` WHERE {$where} ORDER BY `sort`, `id` DESC LIMIT {$pageIndex}, {$pageSize}";
	// echo $sql;
	$res = $db->query($sql)->fetchAll(PDO::FETCH_ASSOC);
	$res = ( $res && is_array($res) ) ? $res : array();
	foreach ( $res as $k => $v )
	{
		$res[$k]['file'] = "http://gt2.youjoy.tv/ddzgamefile/mall_recharge/".$v['fileId'].".png";
	...
	$array['data'] = $res;
	$sql = "SELECT count(*) as cn FROM `lord_list_recharge` WHERE {$where} ";
	...
	$.post("rechargeList.php",{
		...
		if(result!=null && result!=""){
			...
			for ( var i=0; i<dataList.data.length; i++ ) {
				...
				dataListHtml += "<td>"+o.value+(o.type=="money2coins"?"乐豆":"？？")+"</td>";
				dataListHtml += "<td>"+o.price+(o.type=="money2coins"?"元":"？？")+"</td>";
				...
				dataListHtml += "<td>"+(o.state!=2?"<a href='#' onclick='ajaxto(this,\"rechargeAdd.php?type=delete&id="+o.id+"\")'>删除</a>&nbsp;&nbsp;":"")+(o.state==0?"<a href='#' onclick='ajaxto(this,\"rechargeAdd.php?type=offline&id="+o.id+"\")'>下架</a>&nbsp;&nbsp;":"")+(o.state==1?"<a href='#' onclick='ajaxto(this,\"rechargeAdd.php?type=online&id="+o.id+"\")'>上架</a>&nbsp;&nbsp;":"")+"<a href='#' onclick='linkto(this,\"rechargeAdd.php?type=modify&id="+o.id+"\")'>修改</a></td>";
				dataListHtml += "</tr>";
			}
...
<fieldset>
	...
	<div class="row">
		...
		<div span="span1" style="float:right;">
			...
			<input type="button" value="创&nbsp;&nbsp;建" onclick="linkto(this,'rechargeAdd.php')" class="btn"/>
		</div>
	</div>
</fieldset>
