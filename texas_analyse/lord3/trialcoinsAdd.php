...
if ($type=='delete') {
	$sql = "UPDATE `lord_list_trialcoins` SET `is_del` = 1 WHERE `id` = $id";
...
$data = array();
if ($type=='modify') {
	$sql = "SELECT * FROM `lord_list_trialcoins` WHERE `id` = $id";
...
if ($ispost) {
	...
	$probability = isset($_REQUEST['probability'])?intval($_REQUEST['probability']):0;
	$sort = isset($_REQUEST['sort'])&&intval($_REQUEST['sort'])?intval($_REQUEST['sort']):99;
	$create_time = $update_time = $ut_now;
	if ($type=='add') {
		$sql = "INSERT INTO `lord_list_trialcoins` (`channel`,`value`,`multiple`,`probability`,`is_del`,`sort`,`create_time`,`update_time`)
		VALUES ('$channel',$value,$multiple,$probability,0,$sort,$create_time,$update_time)";
	}
	if ($type=='modify') {
		$sql = "UPDATE `lord_list_trialcoins` SET `channel`='$channel',`value`=$value,`multiple`=$multiple,`probability`=$probability,`sort`=$sort,`update_time`=$update_time WHERE `id`=$id";
	}
	$res = $pdo->getDB(1)->exec($sql);
	if ( $res ) {
...
<fieldset>
	<legend>救济乐豆项 - <?php if($type=='add'){?>创建<?php }else{?>编辑<?php }?></legend>
	<form action="trialcoinsAdd.php" method="post">
		...
		<table class="table table-bordered table-condensed table-hover">
			...
			<tr>
				<td style="width:70px;font-size:14px;">救济乐豆:</td>
				<td><input name="value" value="<?php if($data){echo $data['value'];} ?>" type="text" class="in_t" /></td>
			</tr>
			<tr>
				<td style="width:70px;font-size:14px;">乐豆倍率:</td>
				<td><input name="multiple" value="<?php if($data){echo $data['multiple'];} ?>" type="text" class="in_t" /></td>
			</tr>
			<tr>
				<td style="width:70px;font-size:14px;">权重概率:</td>
				<td><input name="probability" value="<?php if($data){echo $data['probability'];} ?>" type="text" class="in_t" /></td>
			</tr>
			<tr>
