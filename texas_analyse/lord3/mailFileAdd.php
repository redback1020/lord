...
if ($ispost) {
	...
	if ( !$errno ) {
		header('Location: mailFileList.php');
	}
	else {
		echo $res;
		exit;
...
<fieldset>
	<legend>邮件图片-<?php if($type=='add'){?>创建<?php }else{?>编辑<?php }?></legend>
	<form action="mailFileAdd.php" method="post">
		...
		<table class="table table-bordered table-condensed table-hover">
			...
			<tr>
				...
				<td>暂不支持，目前需要手动上传到CDN的目录user_inbox下
					<br/>规格：邮件用图 &lt;=473x360
				</td>
			</tr>
			<tr>
