...
if ($isAjax) {
	...
	$where = "t.`state` <> 2 ";
	if ($path == 'all') $where .= " AND (f.`path`='topic_index' OR f.`path`='topic_lobby')";
	if ($path != 'all') $where .= " AND f.`path`='".$path."'";
	if ($version != 'all') $where .= " AND f.`version`={$version}";
	$sql = "SELECT f.*, t.`subject`, t.`sort` FROM `lord_game_file` f LEFT JOIN `lord_game_topic` t ON f.`fileid` = t.`id` WHERE {$where} ORDER BY t.`sort` ASC, f.`fileid` DESC LIMIT {$pageIndex}, {$pageSize}";
	$res = $db->query($sql)->fetchAll();
	...
	$array['data'] = $res;
	$sql = "SELECT count(*) as cn FROM `lord_game_file` f LEFT JOIN `lord_game_topic` t ON f.`fileid` = t.`id` WHERE {$where} ";
	$res = $db->query($sql)->fetch();
	$array['count'] = $res['cn'];
		...
		if(result!=null && result!=""){
			...
			for(var i=0;i<dataList.data.length;i++){
				...
				if(o.ver_del>0){
					...
					dataListHtml += "<td><a href='#' onclick='ajaxto(this,\"topicFileAdd.php?type=delete&id="+o.id+"\")'>删除</a>&nbsp;&nbsp;<a href='#' onclick='linkto(this,\"topicFileAdd.php?type=modify&id="+o.id+"\")'>修改</a><br><a href='#' onclick='viewimg(this,\""+o.path+"/"+o.fileid+"\")'>查看图片</a></td>";
				}
				dataListHtml += "</tr>";
			}
...
<fieldset>
	...
	<div class="row">
		...
		<div span="span1" style="float:right;">
			...
			<input type="button" value="创&nbsp;&nbsp;建" onclick="linkto(this,'topicFileAdd.php')" class="btn"/>
		</div>
	</div>
</fieldset>
