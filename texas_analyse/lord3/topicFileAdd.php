...
if ($type=='delete') {
	...
	if ( $res ) {
		//发布时，才会处理数据向redis同步
		// $res = file_get_contents(API_BASE."file&type={$type}&id={$id}");
...
if ($ispost) {
	...
	if ( !$errno ) {
		header('Location: topicFileList.php');
	}
	else {
		echo $res;
		exit;
...
<fieldset>
	<legend>活动图片-<?php if($type=='add'){?>创建<?php }else{?>编辑<?php }?></legend>
	<form action="topicFileAdd.php" method="post">
		...
		<table class="table table-bordered table-condensed table-hover">
			...
			<tr>
				<td style="width:70px;font-size:14px;">活动编号:</td>
				<td><input name="fileid" value="<?php if($data){echo $data['fileid'];} ?>" type="text" class="in_t2" /></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>暂不支持，目前需要手动上传到CDN，格式及命名为“活动编号数字.png”。
					<br/>活动内容图传到topic_index目录下；热门活动图传到topic_lobby下
					<br/>规格：活动内容图 &lt;=473x445；热门活动图 &lt;=1074x467
				</td>
			</tr>
			<tr>
				<td>&nbsp;</td>
