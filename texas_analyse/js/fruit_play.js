...
app.controller('FruitPlayLogController', function ($scope, $http) {
    ...
    $scope.orderBy = "log_fruit_bet.id";
    ...
    $scope.Refresh = function (page) {
        ...
        if ($scope.playerId != undefined && $scope.playerId != 0) {
            where.push({field: "log_fruit_bet.playerId", value: $scope.playerId, condition: "="});
        }


        if ($scope.playerName != undefined && $scope.playerName != "") {
            where.push({field: "log_fruit_bet.playerName", value: $scope.playerName, condition: "like"});
        }

        var date_ranges = $scope.date_range.split(" - ");

        where.push({field: "log_fruit_bet.logdate", value: date_ranges[0], condition: ">="});
        where.push({field: "log_fruit_bet.logdate", value: date_ranges[1], condition: "<="});
