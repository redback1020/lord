...
$(function(){ 
 ...
 <body>
	...
	$finish = $end ;
	//注册用户数
	$sql = "select count(distinct m.uid) as cn,m.channel from lord_game_analyse a, lord_game_user m where a.uid = m.uid and a.add_time>='".$start."' and a.add_time<'".$end."'".$where." group by channel order by cn desc ";	  
	...
	//次日留存
	if($diff == 1){
		$sql = "select count(distinct m.uid) as cn, channel from lord_log_login m where  login_time>='".$end."' and login_time<'".$start3."' and reg_time>='".$start."' and reg_time<'".$end."'".$where." group by channel";
		...
		//当日转换
		$sql = "select count(distinct m.uid) as cn, channel from lord_log_login m where  login_time>='".$start."' and login_time<'".$end."' and reg_time>='".$start."' and reg_time<'".$end."'".$where." group by channel";
		//echo $sql.'<br>';
		$row = $db -> query($sql)->fetchAll();
		foreach($row as $val){
	...
	//当日注册付费用户
	$sql = "select count(distinct(m.uid)) as cn,m.channel  from  lord_game_charge c, lord_game_user m , lord_game_analyse a where c.uid = m.uid and m.uid = a.uid and a.add_time>='".$start."' and a.add_time<'".$end."'".$where." and c.time>='".$start."' and c.time<'".$end."' group by m.channel";
	...
	//活跃用户数
	$sql = "select count(distinct(m.uid)) as cn, channel from lord_log_login m where  m.login_time>='".$start."' and m.login_time<'".$end."'".$where." group by channel";
	$row = $db -> query($sql)->fetchAll();
	foreach($row as $val){
	...
	//当日所有付费用户
	$sql = "select count(distinct(m.uid))as cn,m.channel from  lord_game_charge c, lord_game_user m  where c.uid = m.uid and c.time>='".$start."' and c.time<'".$end."'".$where." group by m.channel order by cn desc";
	...
	//当日付费总额
	$sql = "select sum(c.gold) as cn,m.channel from lord_game_charge c, lord_game_user m  where c.uid = m.uid and c.time>='".$start."' and c.time<'".$end."'".$where." group by m.channel";
	$row = $db -> query($sql)->fetchAll();
	foreach($row as $val){
