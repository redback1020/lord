...
define('TAG_NAME', 'HOUR_TOTAL_CHANNEL');
...
$dateid = intval($mysql->getVar($sql));
...
//当日注册用户
$DNU = array();
$sql = "select count(distinct m.uid) as cn, m.channel from lord_game_analyse a left join lord_game_user m on a.uid = m.uid where a.add_time >= '".$start."' and a.add_time < '".$end."' group by m.channel order by cn desc ";
$row = $mysql->getData($sql);
foreach ( $row as $val ) {
...
//次日留存用户
$DR1 = array();
$sql = "select count(distinct m.uid) as cn, m.channel from lord_game_login m left join lord_game_analyse a on m.uid = a.uid where m.login_time >= '".$start2."' and m.login_time < '".$end2."' and a.add_time >= '".$start."' and a.add_time < '".$end."' group by m.channel";
$row = $mysql->getData($sql);
foreach ( $row as $val ) {
...
//三日留存用户
$DR2 = array();
$sql = "select count(distinct m.uid) as cn, m.channel from lord_game_login m left join lord_game_analyse a on m.uid = a.uid where m.login_time >= '".$start3."' and m.login_time < '".$end3."' and a.add_time >= '".$start."' and a.add_time < '".$end."' group by m.channel";
$row = $mysql->getData($sql);
foreach ( $row as $val ) {
...
//七日留存用户
$DR6 = array();
$sql = "select count(distinct m.uid) as cn, m.channel from lord_game_login m left join lord_game_analyse a on m.uid = a.uid where m.login_time >= '".$start7."' and m.login_time < '".$end7."' and a.add_time >= '".$start."' and a.add_time < '".$end."' group by m.channel";
$row = $mysql->getData($sql);
foreach ( $row as $val ) {
